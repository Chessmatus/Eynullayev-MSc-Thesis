...
import gfnx
from src.gfnx.environment.dag import DAGEnvironment
from src.gfnx.reward import BDeuScore
from src.gfnx.metrics import (
...
from src.gfnx.utils.dag import get_markov_blanket
...
def load_data_file(filepath):
    """
    Загружает данные из файла формата bnlearn (asia.dat).
    
    Формат файла:
    Строка 1: Имена переменных (например: A  S  T  L  B  E  X  D)
    Строка 2: Кардинальности (например: 2  2  2  2  2  2  2  2)
    Строка 3: Тип (d для discrete, c для continuous)
    Строки 4+: Данные сэмплов
    """
    
    # Загружаем данные (начиная со строки 3)
    data = np.loadtxt(filepath, skiprows=3, dtype=int)
    
    # Конвертируем в JAX массив
    data_jax = jnp.asarray(data, dtype=jnp.int32)
...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    # print(metrics_state['MAD'].empirical_edge_probabilities)
    ...
    metrics_state = metrics_module.update(
        ...
        args=metrics_module.UpdateArgs(
            metrics_args={
                "MAD": MADMetricsModule.UpdateArgs(
    ...
    metrics_state = jax.lax.cond(
        ...
        lambda kwargs: kwargs["metrics_state"],  # Do nothing if not eval step
        {
            ...
            "args": metrics_module.ProcessArgs(
                metrics_args={
                    "MAD": MADMetricsModule.ProcessArgs(
                        env_params=train_state.env_params,
                    )
                }
            ),
...
def run_experiment(cfg: OmegaConf) -> None:
    ...
    # Load the samples
    train_samples = load_data_file(cfg.data.train_data)
    true_edge_probs = jnp.array(pd.read_csv(cfg.environment.true_edge_probs).values)
    ...
    elif cfg.environment.likelihood.type == "bdeu_score":
        likelihood = BDeuScore(
            data=train_samples,
            cardinalities=cfg.environment.likelihood.cardinalities,
            ess=cfg.environment.likelihood.ess,
        )
    else:
    ...
    env = DAGEnvironment(
        reward_module,
        num_variables=cfg.environment.num_variables,
        true_edge_probs=true_edge_probs
    )
    ...
    # log.info("Starting calculation of all possible graphs")
    ...
    # log.info("Started calculation of test_set_states")
    ...
    # )
    # log.info("finished calculation of test_set_states.Started initializing metrics")

    metrics_state = metrics_module.init(
        eval_init_key,
        metrics_module.InitArgs(
            metrics_args={
                "MAD": MADMetricsModule.InitArgs(env_params=env_params),
    ...
    log.info("finished.Started initializing train state")
    ...
    log.info("finished.Started partitioning train state")
